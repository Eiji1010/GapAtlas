...
MVP ではこれを許容する。壊れないようにした手当ては次のとおり。

- 途中経過の概要は **`save_scan_if_unfinished` で条件付きに書く**。確定済みの
  概要を `processing` へ巻き戻さない。無条件上書きだと、遅れて着地した途中
  経過が完了状態を消し、ランキングと Brief が失われ、フロントエンドは終端
  状態へ到達できず 2秒 Polling を続ける
- ランキングは同じ `CountryResult` 群から `_ranking_key` で決まるので同一になる
- Opportunity Brief は **Evidence ハッシュでキャッシュ**する
  (`adapters/llm/cache.py`)。実 LLM は決定的でないため、キャッシュが無いと
  2回目の確定が別の文面で上書きし、Polling 中のユーザーには Brief が
  書き換わって見える

**残るコスト**: Maps の再取得ガード(`source_status[maps]` が
`NOT_REQUESTED` の国だけ取得する)は、片方が**書き終えた後に**もう片方が読む
場合にしか効かない。両方が書き込み前に読むと Top2 分の Maps 呼び出しが
2回ずつ、計4回になる。デモ規模(5か国 / Reserved concurrency 2)では許容
できると判断した。厳密に解くには `ScanRepository` へ確定処理のリーダー選出
(条件付き書き込みによる `finalized_by` の獲得)を足す必要がある。
